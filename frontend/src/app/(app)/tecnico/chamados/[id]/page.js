...
export default function TecnicoChamadoDetalhePage() {
  ...
  async function fetchChamadoById() {
    ...
    setError("");
    try {
      const data = await chamadosService.findById(id);
  ...
  async function handleUpdateStatus(event) {
    ...
    try {
      await chamadosService.updateStatus(id, { status, tecnico_id: user?.id });
  ...
  return (
    <PageSection title={`Atendimento do chamado #${id || "..."}`} description="Atualize o andamento do chamado.">
      {chamado ? (
